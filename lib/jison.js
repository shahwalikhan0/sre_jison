...
Jison.Parser = (function () {
  ...
  lrGeneratorMixin.gotoOperation = function gotoOperation(itemSet, symbol) {
    ...
    itemSet.forEach(function goto_forEach(item, n) {
      if (item.markedSymbol === symbol) {
        gotoSet.push(
          new self.Item(item.production, item.dotPosition + 1, item.follows, n)
        );
      }
    });
  ...
  lrGeneratorMixin.canonicalCollectionInsert =
    function canonicalCollectionInsert(symbol, itemSet, states, stateNum) {
  ...
  lrGeneratorMixin.generateCommonJSModule = function generateCommonJSModule(
    opt
  ) {
    opt = typal.mix.call({}, this.options, opt);
  ...
  function addTokenStack(fn) {
    ...
    try {
      ...
      var labeled = JSONSelect.match(
        ':has(:root > .label > .name:val("_token_stack"))',
        ast
      );
  ...
  lrGeneratorMixin.generateModule_ = function generateModule_() {
    ...
    moduleCode += [
      ...
      "defaultActions: " +
        JSON.stringify(this.defaultActions).replace(/"([0-9]+)":/g, "$1:"),
      "parseError: " +
        String(
          this.parseError ||
      ...
      states.forEach(function union_states_forEach(i) {
        ...
        if (state.reductions.length)
          state.reductions.forEach(function union_reduction_forEach(item) {
            ...
            state.goes[item.production.handle.join(" ")].forEach(
              function reduction_goes_forEach(symbol) {
                newg.nonterminals[symbol].follows.forEach(
                  function goes_follows_forEach(symbol) {
      ...
      do {
        ...
        set.forEach(function (item) {
          ...
          if (symbol && self.nonterminals[symbol]) {
            ...
            if (
              b.length === 0 ||
              item.production.nullable ||
              self.nullable(r)
            ) {
              b = b.concat(item.follows);
            }
            self.nonterminals[symbol].productions.forEach(function (
              production
            ) {
              var newItem = new self.Item(production, 0, b);
              if (
                !closureSet.contains(newItem) &&
